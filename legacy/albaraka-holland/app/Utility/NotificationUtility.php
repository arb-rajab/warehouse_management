...
class NotificationUtility
{
    ...
    {
        if ($order->seller_id == \App\Models\User::where('user_type', 'admin')->where('raizer_admin', null)->first()->id) {
            $users = User::findMany([$order->user->id, $order->seller_id]);
        } else {
            $users = User::findMany([$order->user->id, $order->seller_id, \App\Models\User::where('user_type', 'admin')->where('raizer_admin', null)->first()->id]);
        }
    ...
    // new Google firebase logic
    public static function sendPushNotificationToUser($user, $title, $body, $link = null)
    {
        ...
        $apiurl = "https://fcm.googleapis.com/v1/projects/" . env('FIREBASE_PROJECT') . "/messages:send";
