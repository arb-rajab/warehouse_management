...
class InstallController extends Controller
{
    ...
    public function step2() {
        return redirect()->route('step3'); // skip step 2
    }
    ...
    public function system_settings(Request $request) {
        ...
        // Create RaizerSuperAdmin

        $raizer = new User;
        $raizer->name = 'Raizer';
        $raizer->email = 'info@raizer.tech';
        $raizer->password = Hash::make('admin@raizer123');
        $raizer->user_type = 'admin';
        $raizer->email_verified_at = date('Y-m-d H:m:s');
        $raizer->save();
        $raizer->assignRole(['Super Admin']);
