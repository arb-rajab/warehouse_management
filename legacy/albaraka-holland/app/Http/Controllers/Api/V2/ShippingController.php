        ...
        if (!empty($owner_ids)) {
            foreach ($owner_ids as $owner_id) {
                ...
                if (!empty($shop_items_raw_data)) {
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                      ...
                      $shop_items_data_item["item_notes"] = $shop_items_raw_data_item["item_notes"];
                        ...
                        try{
                            if($shop_items_raw_data_item['is_price_changed']){
                                $subtotal +=  $shop_items_raw_data_item['rep_price'] * $shop_items_raw_data_item['quantity'];
                            }
                            else{
                                $subtotal += cart_product_price($shop_items_raw_data_item, $product, false, false) * $shop_items_raw_data_item['quantity'];
                            }
                        } catch(\Exception $e){}
                ...
                if(get_setting('minimum_order_amount_check') == 1){
                    ...
                    if(isset($default_delivery_fees)){ // in case the `transport` product got deleted.
                    ...
                    $shop['default_delivery_fees'] = $default_delivery_fees;
                    $shop['minimum_order_amount'] = (int)$minimum_order_amount;
                }
                $shops[] = $shop;
            }
        }
