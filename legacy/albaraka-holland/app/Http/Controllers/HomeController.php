...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $todays_deal_products = Cache::rememberForever('todays_deal_products', function () {
            return filter_products(Product::where('published', 1)->where('todays_deal', '1'))->orderBy('name')->get();
        });

        $newest_products = Cache::remember('newest_products', 20, function () {
            return filter_products(Product::where('isNewProduct', true)->where('published', 1))->orderBy('name')->get();
        });

        $offers_catalog = OffersCatalog::first();

        return view('frontend.index', compact('featured_categories', 'todays_deal_products', 'newest_products', 'offers_catalog'));
    }
    ...
    public function registration(Request $request)
    {
        ...
        $countries = Country::where('status', 1)->orderBy('priority')->get();
        return view('frontend.user_registration', compact('countries'));
    }
    ...
    public function profile(Request $request)
    {
        ...
        if (Auth::user()->user_type == 'seller') {
            ...
            return view('frontend.user.profile', compact('countries'));
        }
    }
    ...
    {
        $detailedProduct  = Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop', 'barcodes')->where('auction_product', 0)->where('slug', $slug)->where('approved', 1)->first();
        logProductView($detailedProduct?->id);

        if ($detailedProduct && ProductVariation::where('product_id', $detailedProduct->id)->exists()) {
            $variations = ProductVariation::where('product_id', $detailedProduct->id)->get();
        } else {
            $variations = null;
        }
        if ($detailedProduct != null /*&& $detailedProduct->published*/) {
            ...
            return view('frontend.product_details', compact('detailedProduct', 'product_queries', 'total_query', 'reviews', 'review_status', 'variations'));
        }
        abort(404);
    }
    ...
    public function filter_shop(Request $request, $slug, $type)
    {
        ...
        if ($shop != null && $type != null) {
            ...
            if ($type == 'all-products') {
                ...
                $conditions = ['user_id' => $shop->user->id, 'approved' => 1];
    ...
    public function all_categories(Request $request)
    {
        if(! (Auth::check() && Auth::user()->user_type == 'admin')){
            $categories = Category::where('level', 0)->where('name', '!=', 'Uncategorized')->orderBy('order_level', 'desc')->get();
    ...
    {
        $products = filter_products(Product::where('added_by', 'admin'))->where('published', true)->with('taxes')->paginate(12)->appends(request()->query());
