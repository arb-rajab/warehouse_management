...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <h5 class="mb-0 h6">{{translate('Update Trip')}}</h5>
</div>
...
<div class="col-lg mx-auto">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('trips.update') }}" method="POST">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md">
                        <input type="date" class="form-control aiz-date" name="due_date" id="due_date" placeholder="{{ translate('Select Date') }}"
                            value="{{ old('due_date', $trip->due_date ? \Carbon\Carbon::parse($trip->due_date)->format('Y-m-d') : '') }}"
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm">
                        <textarea type="text" placeholder="{{ translate('Notes')}}" name="notes" class="form-control"
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div id="checkpoints" class="sortable-checkpoints">
                            ...
                            @if (old('checkpoints_array') ?? $trip->checkpoints)
                                @foreach (old('checkpoints_array') ?? $trip->checkpoints as $key => $checkpoint)
                                    @include('backend.trips.components.checkpoint', ['type' => $checkpoint['checkpoint_type'] ?? $checkpoint->type, 'checkpoint' => old('checkpoints_array') ? null : $checkpoint, 'key' => $key])
                                @endforeach
                            @endif
                        </div>
