        ...
        @can('add_offer_deal')
            <div class="col-md-6 text-md-right">
                <a href="{{ route('offer_deals.create') }}" class="btn btn-circle btn-info">
                    <span>{{translate('Create New Offer Deal')}}</span>
                </a>
            </div>
        @endcan
        @can('add_offer_deal')
            ...
            <tbody>
                @foreach($offer as $key => $offer_deal)
                    <tr>
						...
						<td class="text-right">
                            @can('edit_offer_deal')
                                <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('offer_deals.edit', ['id'=>$offer_deal->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
