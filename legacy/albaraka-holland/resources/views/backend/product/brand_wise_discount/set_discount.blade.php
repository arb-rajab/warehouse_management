...
@section('content')
    <div class="aiz-titlebar text-left mt-2 mb-3">
        <div class="row align-items-center">
            <div class="col-md-6">
                <h1 class="h3">{{ translate('Set Brand Product Discount') }}</h1>
            </div>
        </div>
    </div>
    <div class="card">
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="lg" class="text-center">{{ translate('Action') }}</th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($brand->logo != null)
                                    <span class="avatar avatar-square avatar-xs">
                                        <img src="{{ uploaded_asset($brand->logo) }}" alt="{{ translate('logo') }}">
                                    </span>
                                @else
                                    —
                                @endif
                            ...
                            <td>
                                <div class="input-group">
                                    <input type="number" class="form-control" id="discount_{{ $brand->id }}"
                                        step="0.01" value="0" min="0"
                                        placeholder="{{ translate('Discount') }}" style="border-radius: 8px 0 0 8px;">
                            ...
                            <td>
                                <input type="text" class="form-control aiz-date-range rounded-2"
                                    id="date_range_{{ $brand->id }}" placeholder="{{ translate('Select Date') }}"
                                    data-time-picker="true" data-format="DD-MM-Y HH:mm:ss" data-separator=" to "
                                    autocomplete="off">
                            </td>
                            <td class="text-center">
                                <div class="form-group mb-0 text-center">
                                    <button type="button" onclick="trigger_alert({{ $brand->id }})"
...
@section('script')
    <script type="text/javascript">
        ...
        function trigger_alert(brandId) {
            $('#trigger_btn').attr('data-value', brandId);
            $('#confirm-modal').modal('show');
        }

        function setDiscount() {
            $('#confirm-modal').modal('hide');
            var brandId = $('#trigger_btn').attr('data-value');
            var discount = $("#discount_" + brandId).val();
            var dateRange = $("#date_range_" + brandId).val();
            var sellerProductDiscount = $("#seller_product_discount_" + brandId).prop('checked') ? 1 : 0;
                ...
                $.post('{{ route('set_product_discount') }}', {
                    ...
                    if (data == 1) {
                        AIZ.plugins.notify('success',
                            '{{ translate('brand Wise Product Discount Set Successfully') }}');
                    }
                    location.reload();
                }).fail(function() {
