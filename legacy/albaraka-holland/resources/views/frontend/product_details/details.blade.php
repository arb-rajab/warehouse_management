<div class="text-left">
    ...
    <h1 class="mb-4 fs-16 fw-700 text-dark">
        {{ $detailedProduct->name . ' | ' . $detailedProduct->ar_name }}
    </h1>
    ...
    <p class="my-3 fs-14 fw-700">Material ID: {{ $detailedProduct->mat_id }}</p>

    <!-- Profits Stars -->
    @auth
        @if(auth()->user()->is_rep || auth()->user()->user_type == 'admin')
            <div class="col-12">
                <span class="rating rating-mr-1">
                    {{ renderStarRating($detailedProduct->collection?->amount ?? 0) }}
                </span>
            </div>
        @endif
    @endauth
        ...
        @else
            ...
            @if (home_price($detailedProduct) != home_discounted_price($detailedProduct))
                @if (Auth::check() && Auth::user()->admin_verified)
                        ...
                        <div class="col-sm-10">
                            <div class="d-flex align-items-center">
                                ...
                                @if ($detailedProduct->unit != null)
                                    ...
                                    {{-- <span class="opacity-70"> / {{ $detailedProduct->unit_equal }}</span> --}}
                                @endif
                    ...
                    <div class="row no-gutters mb-3">
                        <div class="col-sm-2">
                            <div class="text-secondary fs-14 fw-400">{{ translate('Price Per Piece') }}</div>
                        </div>
                        <div class="col-sm-10">
                            <strong
                                class="fs-16 fw-700 text-primary">{{ '€' . $detailedProduct->unit_price / $detailedProduct->unit_equal }}</strong>
                ...
                {{-- Show Unit & Unit Equal  --}}
                @if ($detailedProduct->unit != null)
                    <div class="row no-gutters mb-3">
                        <div class="col-sm-2">
                            <div class="text-secondary fs-14 fw-400">{{ translate('Unit') }}</div>
                        </div>

                        <div class="col-sm-10">
                            <span> {{ $detailedProduct->unit_equal }}</span>
                            <span class="ml-1">{{ $detailedProduct->getTranslation('unit') }}</span>
                        </div>
            ...
            @else
                ...
                @if (Auth::check() && Auth::user()->admin_verified)
                    ...
                    @if ($detailedProduct->unit_equal > 0)
                        <div class="row no-gutters mb-3">
                            <div class="col-sm-2">
                                <div class="text-secondary fs-14 fw-400">{{ translate('Price Per Piece') }}</div>
                            </div>
                ...
                @if ($detailedProduct->unit != null)
                    <div class="row no-gutters mb-3">
                        ...
                        <div class="col-sm-10">
                            <strong
                                class="fs-16 fw-700 text-primary">{{ $detailedProduct->unit_equal . ' pieces / ' . $detailedProduct->unit }}</strong>
                        </div>
                    </div>
                @endif
                                ...
                                <div class="d-flex align-items-center">
                                    <div class="row no-gutters align-items-center mr-3" style="width: 130px;">
                                        <button class="btn col-auto btn-icon btn-sm btn-light rounded-0"
                                            type="button" data-type="minuss" {{-- data-field="variation-{{ $variation->id }}" --}}>
                                            <i class="las la-minus"></i>
                                        </button>
                                        <input type="number" name="variation[{{ $variation->name }}]"
                                            class="col border-0 text-center flex-grow-1 fs-16 input-number"
                                            placeholder="1" lang="en" value="0" min=0>
                                        <button class="btn col-auto btn-icon btn-sm btn-light rounded-0"
                                            type="button" data-type="pluss" {{-- data-field="variation-{{ $variation->id }}" --}}>
                        ...
                        <div class="col-sm-10">
                            <div class="product-quantity d-flex align-items-center">
                                <div class="row no-gutters align-items-center aiz-plus-minus mr-3"
                                    ...
                                    <input type="number" name="quantity" id="total-quantity"
                                        class="col border-0 text-center flex-grow-1 fs-16 input-number"
                                        placeholder="1" value="{{ $detailedProduct->min_qty - 1 }}" readonly
                                        {{-- min="{{ $detailedProduct->min_qty }}" max="{{ $detailedProduct->max_qty }}" --}} lang="en">
                                    {{-- <button class="btn col-auto btn-icon btn-sm btn-light rounded-0" type="button"
                            ...
                            <div class="product-quantity d-flex align-items-center">
                                <div class="row no-gutters align-items-center aiz-plus-minus mr-3"
                                    ...
                                    <input type="number" name="quantity"
                                        ...
                                        min="{{ $detailedProduct->min_qty }}" max="{{ $detailedProduct->max_qty }}"
                                        lang="en">
                                    <button class="btn col-auto btn-icon btn-sm btn-light rounded-0" type="button"
                                        data-type="plus" data-field="quantity">
                                        <i class="las la-plus"></i>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const plusButtons = document.querySelectorAll('[data-type="pluss"]');
        const minusButtons = document.querySelectorAll('[data-type="minuss"]');
        const totalQuantityInput = document.getElementById('total-quantity');

        const variationInputs = document.querySelectorAll('[name^="variation"]');
        ...
        plusButtons.forEach(function(plusButton) {
            plusButton.addEventListener("click", function() {
                const inputField = this.closest('.row').querySelector('.input-number');
