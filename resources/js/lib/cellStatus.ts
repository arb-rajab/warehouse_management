...
/**
 * Whether a cell's pallet expires within `withinDays` days of `today` (an
 * already-expired pallet also matches, since its expiration is trivially
 * "within" any window) — anchored on the server-provided `today`, never the
 * browser clock. There is no default threshold: `withinDays === null` means
 * "not checking for expiry", so this always returns false.
 */
export function isCellExpiringWithin(
    ...
    return cell.pallet.expiration_date <= addDays(today, withinDays);
}
