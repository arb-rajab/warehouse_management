<div class="modal-body px-4 py-5 c-scrollbar-light">
    ...
    @php
        $featuredRelatedProduts = filter_products(\App\Models\Product::where('category_id', $product->category_id)->where('id', '!=', $product->id))->where('featured', true)->limit(10)->get();
    @endphp

    @if ($featuredRelatedProduts->count() > 0)
        <!-- Related product -->
        <div class="bg-white shadow-sm">
            ...
            <div class="p-3">
                <div class="aiz-carousel gutters-5 half-outside-arrow" data-items="2" data-xl-items="3" data-lg-items="4"  data-md-items="3" data-sm-items="2" data-xs-items="2" data-arrows='true' data-infinite='true'>
                    @foreach ($featuredRelatedProduts as $key => $related_product)
                    <div class="carousel-box hov-scale-img hov-shadow-sm">
                        <div class="aiz-card-box my-2 has-transition">
                            ...
                            <div class="p-md-3 p-2 text-center">
                                ...
                                @if(Auth::check() && Auth::user()->admin_verifed)
    ...
    @endif
    <div class="text-center text-success mb-4">
        <h3 class="fs-20 fw-500">{{ $offer_message }}</h3>
    </div>
