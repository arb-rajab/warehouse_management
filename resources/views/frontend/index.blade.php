...
@if($offers_catalog)
    <div class="mx-5 my-5">
            <div class="col justify-content-center">
                <a class="btn btn-info btn-block " href="{{ route('offers.download') }}">Download Offers Catalog </a>
            </div>
    </div>
@endif
 ...
 <div id="section_newest">
  ...
  <div id="section_featured">
                        ...
                        @foreach ($featured_categories as $key => $category)
                            <div class="col-xl-4 col-md-6 border-right border-bottom">
                                <div class="bg-white pt-3 pl-3 pr-3 h-100 hov-scale-img" style="min-height: 240px !important;">
                                    <div class="row">
                                        <!-- Category Banner -->
                                        <a class="text-reset fw-700 fs-14 hov-text-primary" href="{{ route('products.category', $category->slug) }}">
                                        <div class=" h-300px text-center h-md-100">
                                            <img src="{{ uploaded_asset($category->banner) }}" alt="{{ $category->getTranslation('name') }}" class="h-100 has-transition"
                                            onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';" style="width: 100%; object-fit: contain;">
                                        </div>
                                    </a>
                                        <!-- Categories -->
    ...
    @auth
        @if (!auth()->user()->is_rep)
            @php
                $dynamic_popups = App\Models\DynamicPopup::where('status', 1)->forAuthUser()->where('show_page', 0)->get();
            @endphp
            @foreach ($dynamic_popups as $key => $dynamic_popup)
                <div class="modal website-popup removable-session d-none" data-key="website-popup-{{ $dynamic_popup->id }}" data-value="removed" style="display: none;">
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-dialog-zoom modal-md mx-4 mx-md-auto">
                        <div class="modal-content position-relative border-0 rounded-0">
                            ...
                            <button class="close-modal absolute-top-right bg-white shadow-lg btn btn-circle btn-icon mr-n3 mt-n3" data-popup-id="{{ $dynamic_popup->id }}" data-key="website-popup-{{ $dynamic_popup->id }}" data-value="removed" data-toggle="remove-parent" data-parent=".website-popup">
    ...
    <script>
        @auth
            @if (!auth()->user()->is_rep)
                @foreach ($dynamic_popups as $key => $dynamic_popup)
                    setTimeout(function() {
                        $('div[data-key="website-popup-{{ $dynamic_popup->id }}"]').removeClass('d-none').fadeIn();
                    }, {{ $dynamic_popup->delay_seconds * 1000 }}); // Convert seconds to milliseconds
                    @break
                @endforeach
