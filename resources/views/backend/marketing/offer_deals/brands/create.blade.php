        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('offer_deals.store') }}" method="get">
                    ...
                    <input type="hidden" name="brand_offer" value="true">
                    ...
                    <div class="form-group row mb-3">
                        <label class="col-sm-3 control-label" for="brand">{{translate('brand')}}</label>
                        <div class="col-sm-9">
                            <select name="brand" id="brand" class="form-control aiz-selectpicker" required data-placeholder="{{ translate('Choose Products') }}" data-live-search="true" data-selected-text-format="count">
                                <option ></option>
                                @foreach(\App\Models\Brand::orderBy('created_at', 'desc')->get() as $brand)
                    ...
                    <div class="form-group" id="discount_table_for_brand">
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('#brand').on('change', function(){
                ...
                if(brand.length > 0){
                    $.get('{{ route('offer_deals.base_brand_discount') }}', {_token:'{{ csrf_token() }}', brand_id:brand}, function(data){
                        $('#discount_table_for_brand').html(data);
                        AIZ.plugins.fooTable();
                    });
                }
                else{
                    $('#discount_table_for_brand').html(null);
                }
            });
