                    ...
                    <div class="row gutters-16">
                        <div class="col-lg-8">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <input type="number" min="0" class="form-control" name="delay_seconds" value="{{ $dynamic_popup->delay_seconds }}" placeholder="{{ translate('Type your time in seconds') }}" required>
                                </div>
                            </div>
                            <!-- Offer  -->
                            <div class="form-group row">
                                ...
                                <div class="col-lg-8">
                                    <select name="offer_id" id="offer" class="form-control aiz-selectpicker" required>
                                        ...
                                        @foreach ($offers as $offer)
                                            <option value="{{ $offer->id }}" @if($offer->id == $dynamic_popup->offer_id) selected @endif>{{ $offer->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-8">
                                    <select name="show_page" id="show_page" class="form-control aiz-selectpicker" required>
                                        <option value="">{{translate('Select One') }}</option>
                                        <option value="0" @if ($dynamic_popup->show_page == 0) selected @endif>{{translate('home') }}</option>
                                        @php
                                            $categories = \App\Models\Category::all();
                                        @endphp
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}" @if ($dynamic_popup->show_page == $category->id) selected @endif>{{ $category->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
