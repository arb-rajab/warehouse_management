    ...
    <div class="row">
        <div class="col-lg-10 mx-auto">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('new.feat-order.trip.update', $order->id) }}" method="POST">
                        @method('put')
                        @csrf
                        ...
                        <div class="flex-row">
                          ...
                          <textarea placeholder="{{ translate('Notes') }}" id="additional_info" name="additional_info"
                            class="form-control">{{ $order->additional_info }}</textarea>
    ...
    @foreach ($order->orderDetails as $detail)
        <tr class="order-row">
            <td style="max-width:8rem">
                <select class="form-control product-search" disabled>
            ...
            <td class="product-matId">{{ $detail->product_matId }}</td>
            <td class="product-qty" dir="ltr" contenteditable="true">{{ $detail->quantity }}</td>
            <td class="product-unit" style="max-width:5rem">
                <div class="d-flex flex-row-reverse align-middle align-items-center"
                    ...
                    <div class="custom-control custom-switch d-flex align-items-center p-0"
                        ...
                        <input type="checkbox" class="custom-control-input unit-toggle" id="unit_{{ $loop->index }}"
                            name="unit_{{ $loop->index }}" {{ $detail->variation == 'PerBox' ? 'checked' : '' }}>
                        <label class="custom-control-label" for="unit_{{ $loop->index }}">
                            <span class="unit-label ms-2">Box</span>
                        </label>
                    </div>
                    <div>
                        <span class="mx-2">Piece</span>
                    </div>
                </div>
            </td>
            <td class="product-price" dir="ltr" contenteditable="true">{{ $detail->price }}</td>
            <td class="product-total">{{ number_format($detail->price * $detail->quantity, 2) }}</td>
            <td class="product-tax">{{ $detail->tax }}</td>
            <td class="product-tax-total">{{ number_format($detail->tax * $detail->quantity, 2) }}</td>
            <td class="product-net">{{ number_format(($detail->price + $detail->tax) * $detail->quantity, 2) }}</td>
            <td class="product-info" contenteditable="true" style="max-width:5rem">{{ $detail->item_notes }}</td>
            <td>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#save-order').click(function() {
                ...
                $.ajax({
                    url: '{{ route('new.feat-order.trip.update', $order->id) }}',
                    type: 'PUT',
