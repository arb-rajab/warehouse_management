                ...
                @if ($order->seller_id == $admin_user_id || get_setting('product_manage_by_admin') == 1)
                    ...
                    <div class="col-md-3 ml-auto">
                        ...
                        @if (auth()->user()->can('update_order_delivery_status') &&
                            ...
                            <select class="form-control aiz-selectpicker" data-minimum-results-for-search="Infinity" disabled
                ...
                <div class="col-md-4 ml-auto">
                    <table>
                        <tbody>
                            ...
                            @if($order->by_rep && $order->rep != null)
                            <tr>
                                <td class="text-main text-bold">{{ translate('Customer Otajer ID') }}</td>
                                <td class="text-right">{{ $order->rep->AccSysID }}</td>
                            </tr>
                            @endif
                    ...
                    <table class="table-bordered aiz-table invoice-summary table">
                        <thead>
                            <tr class="bg-trans-dark">
                                ...
                                <th>{{ translate('Mat_ID') }}</th>
                                <th class="text-uppercase">{{ translate('Description') }}</th>
                                <th class="text-uppercase">{{ translate('Item Notes') }}</th>
                                <th class="text-uppercase">{{ translate('Variation') }}</th>
                                ...
                                <th data-breakpoints="lg" class="min-col text-uppercase text-center">
                                    {{ translate('Product Original Price') }}</th>

                                <th data-breakpoints="lg" class="min-col text-uppercase text-right">
                                    {{ translate('Edit Item Price') }}</th>
                            ...
                            @foreach ($paginator as $key => $orderDetail)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <strong class="text-danger">{{ translate('Product Unavailable (Deleted)') }}</strong>
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-center">
                                        {{ single_price($orderDetail->profits) }}
                                    </td>

                                    <td class="text-center  text-danger">
                                        @if($orderDetail->is_price_changed)
                                        {{ single_price($orderDetail->original_price) . " ( " . single_price($orderDetail->original_price * $orderDetail->quantity) . " )"  }}
                                        @else
                                            <span class="text-danger">No Change</span>
                                        @endif
                                    </td>

                                    @if($orderDetail->is_price_changed)
                                    <td class="text-center">
                                        <button type="button" data-item-id-price="{{ $orderDetail->id }}" data-item-quantity="{{ $orderDetail->quantity }}" data-original-price="{{ $orderDetail->price }}" class="btn btn-info" onclick="openEditPriceModal(this)">Edit Price</button>
                                    </td>
                                    @endif
...
@section('script')
    <script type="text/javascript">
        ...
        var originalPrice = button.getAttribute('data-original-price');
        var quantity = button.getAttribute('data-item-quantity');
        document.getElementById('update-price').value = originalPrice /quantity;
    ...
    function update_price(id){
        ...
        $.post('{{ route('order.item.edit_price') }}', {_token:'{{ csrf_token() }}', id:itemId, admin_price:updatedPrice }, function(data){
            if(data == 1){
                ...
                AIZ.plugins.notify('danger', '{{ translate('Price can not be more than original price') }}');
                location.reload();
            }
        });
    }
