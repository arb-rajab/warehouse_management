...
This app has an in-app, non-technical help section for admin-panel users, separate from the developer-facing README/CLAUDE.md/`.ai/rules` docs: `routes/web.php`'s `admin.help.index` (`GET admin/help`, `HelpController::index`) and `admin.help.show` (`GET admin/help/{topic}`, `HelpController::show`, `{topic}` bound to the `App\Enums\HelpTopic` backed enum — `rows`/`users`/`products`/`cell-logs`), rendering `resources/js/pages/Admin/Help/Index.vue` (the topic-picker landing page) and one static, no-backend-props page per topic (`Rows.vue`, `Users.vue`, `Products.vue`, `CellLogs.vue`). Every admin listing page that has a matching help topic links to it via the shared `components/HelpLink.vue` ("?" icon) placed in its `PageHeader` slot — see `Rows/Index.vue`, `Users/Index.vue`, `Products/Index.vue`, `CellStatusLogs/Index.vue`.

**Whenever a new admin-facing feature/screen is added to this app, add its corresponding `Admin/Help/<Topic>.vue` sub-page (in plain, non-technical language) plus a `HelpLink` "?" icon on the new screen deep-linking to it (add a new `HelpTopic` case + route/controller branch if it's a genuinely new topic) — in the same change that adds the feature, not as a follow-up.** When help content needs to describe something that already has a single source of truth elsewhere in the app (e.g. cell-state colors/labels), reuse that source (see `Help/CellLogs.vue`'s legend, built from `lib/cellStateColor.ts`'s `CELL_STATE_COLOR`/`CELL_STATES`/`cellStateLabel`) rather than duplicating hardcoded values that could drift from the real UI.
