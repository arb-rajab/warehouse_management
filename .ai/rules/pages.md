...
Link to the specific page the rest of the app already uses for that resource — don't assume "show" is right just because it exists. Workers/users link to their **edit** page (`editUser({id})` from `Admin/UserController`), not a show page, because that's the established target everywhere a user is linked from a table or summary field. Only add the link when a target page actually exists for that resource — don't invent routes.
