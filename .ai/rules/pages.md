...
A worker/user link goes to their **show** page (`showUser({id})` from `Admin/UserController`), not the edit page — see `CellStatusLogs/Index.vue`'s "done by" column and `CellVerificationRounds/Index.vue`/`Show.vue`'s worker link, all consistently pointed at show. Only add the link when a target page actually exists for that resource — don't invent routes.
