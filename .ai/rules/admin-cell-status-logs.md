...
## expires_within_days mirrors created_within_days's mutual-exclusion pattern
`expires_within_days` (nullable|integer|min:1) is the expiration-range counterpart to `created_within_days`: it's an alternative way to express `expiration_date_from`/`expiration_date_to`, not a third independent constraint — `'expires_within_days' => [..., 'prohibits:expiration_date_from,expiration_date_to']` in FiltersCellStatusLogs. In `CellStatusLog::filtered()` it's applied inside the same `whereHas('pallet', ...)` closure as the expiration date-range filters, mapping to `whereDate('expiration_date', '<=', now()->addDays($days))`. Frontend: Index.vue disables the expiration date-range fields while `expires_within_days` has a value and vice versa, same as the created_within_days/date_from/date_to pair.
