...
Don't scope `cellHighlightSamples` by product/flat query params — it's intentionally unfiltered so all the highlight-filter dimensions (including client-only ones like staleAfterDays and multi-select state) can be applied purely client-side, consistent with the existing "matching is never wired into router.get" rule in lib.md.
