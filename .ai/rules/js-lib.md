...
The Cells/Index.vue free-text search box only resolves row-letter+cell-number(+flat) locations (CellController::resolveLocationSearch) — there is deliberately no product-name search and no "jump to next expired pallet" button/query param; this was a deliberate removal, don't reintroduce them.
