...
Both panels anchor at their trigger's inline-start via plain absolute-position static placement (no `start-0`/`end-0`) and size to content with `w-max min-w-full`, uncapped by a fixed `max-w-xs` — that let the panel overflow past the page's physical left edge under `dir="rtl"` for a trigger sitting near that edge (e.g. Dashboard's `PageHeader` slot, which `justify-between` pushes to the physical left in RTL). `usePanelMaxWidth(containerRef)` (lib/useDismissibleListbox.ts) caps the panel to whatever room is actually left in the direction it grows, recomputed on open. Any new dropdown built on this same static-position + content-sized-panel pattern needs the same call — don't reintroduce an uncapped `max-w-xs`.
