...
`config('waf.skip_input_keys')` is the narrower knob for a single *field* rather than a whole route: the listed keys' values are left out of the inspection subject (their key names are not), matched case-insensitively against the last segment of the dotted key so `users.0.password` counts. It holds exactly `password`, `password_confirmation`, `current_password`. Reason: production `Password::defaults()` requires symbols, so a password-manager value containing a backtick or `$(...)` matched the command-injection signatures and produced a bare 403 before routing — no validation error the Inertia form could render. The path-level alternative would have meant excluding admin/login, admin/users and password/change, i.e. switching the input checks off on the three most sensitive routes. Keep this list to credentials; anything wider is an exclude_paths decision, not a skip-key one. Note a value sent as an array (`password[]`) flattens to `password.0` and is still scanned, which is the safe direction.
...
### Substituting the `ajax()` check still needs a positive "this is a page" test
The middleware's condition must keep `isPageVisit()`: an Inertia visit (`X-Inertia` header), or a non-XHR request that accepts HTML. Without it the middleware only *excludes* partial reloads/prefetches/precognitive requests and never requires a page visit at all, so any same-origin XHR GET on a web route overwrote `_previous.url` — strictly more permissive than the `StartSession::storeCurrentUrl()` it replaces, which `ajax()` would have excluded.

The live case: `resources/js/lib/useProductSearch.ts` uses Inertia v3's `useHttp`, whose transport is `XhrHttpClient` (`node_modules/@inertiajs/core/dist/index.js`) — it sets `X-Requested-With` and `X-XSRF-TOKEN` and never `X-Inertia`, and `Admin\ProductController::search()` returns a bare array (JSON), not an Inertia response. ProductSelect's GET `/admin/products/search` therefore became the previous URL, and the next failed pallet submission's `back()` landed the admin on raw JSON with their input lost. Any future JSON-over-XHR endpoint on a web route is the same trap, which is why the check is a header/Accept test rather than a path list. Tests: `tests/Feature/StoreInertiaPreviousUrlTest.php`.
...
## EnsurePasswordHasBeenChanged is web-only on purpose
It is appended to the `web` group in bootstrap/app.php and deliberately not to `api`. `Admin\UserController::store()`/`update()` set `must_change_password` on every created user and admin password reset, but `Api\V1\AuthController::login()` issues a Sanctum token without consulting the flag, so a worker can keep using the mobile app with an admin-issued temporary password. Enforcing it on `api/v1/*` requires a mobile change-password endpoint to escape to (the only one today is web `password/change`), and the mobile client is not in this repository — enforcement without it would lock every flagged worker out of the app. Reviewed and kept as-is; revisit only together with a mobile release that can call such an endpoint. Pinned by 'the mobile api deliberately does not enforce must_change_password' in tests/Feature/Api/V1/AuthControllerTest.php, so lifting the exemption cannot happen silently.
