...
`automaticallyEagerLoadRelationships(false)` is set alongside them, non-production only. Laravel resolves an unloaded relation by eager-loading it across the result set instead of raising the violation, which leaves `preventLazyLoading()` reporting as enabled while never firing — the flag and the behaviour disagree. Production keeps the framework default, since auto-resolving is the safer behaviour for a real user; dev and CI turn it off so the missing eager load surfaces. If the lazy-loading test ever starts passing vacuously, check this first.

Do not collapse the guards back into `Model::shouldBeStrict()`. That helper also enables `automaticallyEagerLoadRelationships()`, which resolves an unloaded relation on access instead of raising the violation — so the lazy-loading guard reports as enabled and silently never fires. That cost a CI round to diagnose: `preventSilentlyDiscardingAttributes` was clearly active (a real MassAssignmentException surfaced) while the lazy-loading test passed through untouched. `tests/Feature/EloquentStrictModeTest.php` now asserts each flag directly *and* behaviourally, so a future bundle change can't quietly disable one again.
