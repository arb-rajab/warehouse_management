...
## Eloquent strict mode is on in local and testing only, via Model::shouldBeStrict()
`AppServiceProvider::configureDefaults()` calls `Model::shouldBeStrict(app()->environment('local', 'testing'))`, alongside the other environment-gated defaults there (`DB::prohibitDestructiveCommands`, `Password::defaults`). In Laravel 13 that helper sets exactly three guards — `preventLazyLoading`, `preventSilentlyDiscardingAttributes`, `preventAccessingMissingAttributes` — turning three rules this codebase already followed by convention into enforced ones: no lazy loading, no reading an attribute a narrow `select()` left out, and no silently dropping a non-fillable attribute on `fill()`/`create()`. It does *not* enable `automaticallyEagerLoadRelationships()`; that stays at the framework default (off), where it is only consulted in `Model::__wakeup()`.

The check names the environments strict mode is **on** for, not the ones it is off for. That is deliberate and worth keeping: staging runs `APP_ENV=staging`, so the obvious-looking `! app()->isProduction()` leaves all three guards armed in front of real users, where a missed eager load should degrade to a slow page rather than a 500. A whitelist also makes any environment added later default to off, which is the safe direction to be wrong in. Note the other environment-gated defaults in this same method still use `isProduction()` — `DB::prohibitDestructiveCommands` and `Password::defaults` are deliberately relaxed on staging, which is a different intent, not an inconsistency to tidy up.

`phpunit.xml` sets `APP_ENV=testing`, so CI runs with all three on and a violation fails the suite. `tests/Feature/EloquentStrictModeTest.php` asserts each guard directly *and* behaviourally, and covers the staging gate by booting the provider under `app()['env'] = 'staging'`, so neither the bundle's contents nor the environment list can change unnoticed.
