...
## Stale pallets have no model-level fixed threshold — every caller supplies the day count
`Pallet::STALE_AFTER_DAYS` (and the `is_stale` attribute/`stale()` scope built on it) were removed entirely — `Pallet::isStaleAfter(int $days): bool` is the single source of truth for the computation and stays parameter-only, with no constant of its own. `CellResource`'s `is_stale` field reads `$request->integer('stale_after_days')` per-request and is `null` when that param is absent, not `false`. Any new stale-related feature must accept an explicit day count from its caller rather than adding a constant to this model.

The one sanctioned exception is a controller-level UI default for a specific caller-adjustable card, the same way `ExpiringSoonDefaults::CUSTOM_WINDOW_DAYS` (45) already exists for the dashboard's custom expiring-days card without `Pallet` having any expiring-related constant. The dashboard's custom "stale within days" card (see dashboard.md) follows the identical shape: `App\Http\Controllers\Concerns\StaleSoonDefaults::CUSTOM_WINDOW_DAYS` (21, re-exposed as `BuildsDashboardStats::DEFAULT_CUSTOM_STALE_DAYS`) is that one card's pre-filled default, read from a `stale_days` request param the same way `expiring_days` already is — it is not a `Pallet`-level constant and does not change `Pallet::isStaleAfter()`'s signature or `CellResource`'s per-request `is_stale`. Don't reintroduce a `Pallet::STALE_AFTER_DAYS`-style constant on the model itself; a new caller-adjustable UI surface gets its own `*Defaults` class instead, same as this one.

In tests, use `Pallet::factory()->stale()` (backdates `created_at` by a fixed, generously-old 30 days in `afterCreating`) as a pallet old enough to read as stale under any reasonable threshold, instead of hand-rolling a `backdate()` call — but note `StaleSoonDefaults::CUSTOM_WINDOW_DAYS` (21) was deliberately chosen below that 30-day backdate so a `stale()` fixture also reads as stale under the dashboard's own default without landing exactly on the day/time-of-day boundary; don't raise that default to 30 without re-checking every test that relies on the gap.
