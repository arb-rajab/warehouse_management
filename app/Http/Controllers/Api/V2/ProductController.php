...
use App;
use App\Models\ProductCollection as ModelsProductCollection;

class ProductController extends Controller
{
    public function index()
    {
        return new ProductMiniCollection(Product::latest()->orderBy('published', 'desc')->paginate(10));
    }
    ...
    public function newProducts()
    {
        ...
        return new ProductMiniCollection(Product::where('isNewProduct', true)->orWhere('isOfferProduct', true)
            ->orderByRaw("CASE
            WHEN name LIKE ' %' THEN 2
            WHEN name LIKE '.%' THEN 0
            ELSE 1
            END" . (empty($order) ? '' : ", $order"), [])
            ->paginate(10));
    }
    ...
    public function category($id, Request $request)
    {
        ...
        // Order by the related `amount` field in `product_collections`
        $products = $products->orderByDesc(
            ModelsProductCollection::select('amount')
                ->whereColumn('product_collections.id', 'products.collection_id')
                ->limit(1)
        );
    ...
    {
        $productBarcodes = ProductBarcode::where('barcode', $request->barcode)->get();
        $productToShow = null;
        if ($productBarcodes->isNotEmpty()) {
            foreach ($productBarcodes as $index => $productBarcode) {
                $product = Product::where('id', $productBarcode->product_id)->where('published', 1)->take(1)->get();
                if ($product->isNotEmpty()) {
                    if ($product[0]->unit == 'BOX') {
                        return new ProductDetailCollection($product);
                    } else {
                        $productToShow = $product;
                    }
                } else {
                    if ($index == count($productBarcodes) - 1 && $productToShow == null) { // Check if it's NOT the last iteration of the loop
                        return response()->json(
                            [
                                'success' => false,
            ...
            if ($productToShow != null) {
                return new ProductDetailCollection($productToShow);
            }
        } else {
            return response()->json(
                [
                    'success' => false,
                    'message' => "The barcode not found",
                    'data' => [],
                ]
            );
        }
    ...
    //     // Apply sorting after pagination
    //     switch ($sort_by) {
    //         case 'price_low_to_high':
    //             $paginatedProducts = $paginatedProducts->sortBy('unit_price');
    //             break;

    //         case 'price_high_to_low':
    //             $paginatedProducts = $paginatedProducts->sortByDesc('unit_price');
    //             break;

    //         case 'new_arrival':
    //             $paginatedProducts = $paginatedProducts->sortByDesc('created_at');
    //             break;

    //         case 'popularity':
    //             $paginatedProducts = $paginatedProducts->sortByDesc('num_of_sale');
    //             break;

    //         case 'top_rated':
    //             $paginatedProducts = $paginatedProducts->sortByDesc('rating');
    //             break;

    //         default:
    //             $paginatedProducts = $paginatedProducts->sortBy('name');
    //             break;
    //     }

    //     // Return direct list of products instead of indexed response
    //     return response()->json([
    //         'data' => new ProductMiniCollection(filter_products($paginatedProducts)),  // Only the products themselves, no index
    //         'success' => true,
    //         'status' => 200,
    //     ]);
    // }
