...
use App\Http\Controllers\CartOfferedController;
...
// Activate the customer account after he agrees on the terms sent by email
    Route::get('/terms-activation/{code}', 'account_activation')->name('terms.activation');
    Route::get('/customers/terms/sendMail/{id}', 'terms_agreements')->name('customers.sendAgreement')->middleware('admin'); // send the email
...
Route::controller(CartOfferedController::class)->group(function () {
    Route::post('/cart-offered/postpone', 'postpone')->name('cart-offered.postpone');
    Route::post('/cart-offered/removeOffersFromCart', 'removeOffersFromCart')->name('cart-offered.removeOffersFromCart');
...
Route::controller(CartController::class)->group(function () {
    ...
    Route::post('/cart/update_item_notes', 'update_item_notes')->name('cart.update_item_notes');
    Route::post('/cart/update_item_price', 'update_item_price')->name('cart.update_item_price');
});
...
Route::group(['middleware' => ['customer', 'verified', 'unbanned']], function() {
    ...
    Route::group(['prefix' => 'checkout'], function() {
        Route::controller(CheckoutController::class)->group(function () {
            ...
            Route::get('/choose_customer', 'get_customers_to_choose')->name('checkout.getCustomers');
            Route::post('/choose_customer', 'choose_customer')->name('checkout.chooseCustomer');
    ...
    // Create a customer by representative
    Route::get('/create_customer', [CustomerController::class, 'create_customer'])->name('create_customer');
    Route::post('/store_customer', [CustomerController::class, 'store_customer'])->name('store_customer');

    Route::get('/representative/payments/index', [RepresentativePaymentController::class, 'index'])->name('payments.index');
    Route::get('/representatives/payments/create', [RepresentativePaymentController::class, 'create'])->name('payments.create');
    Route::post('/representatives/payments/store', [RepresentativePaymentController::class, 'store'])->name('payments.store');

    Route::get('/representatives/payments/show/{id}', [RepresentativePaymentController::class, 'show'])->name('payments.show')->withoutMiddleware(['customer', 'verified', 'unbanned']);
    Route::get('/representatives/payments/export/{id}', [RepresentativePaymentController::class, 'export'])->name('payments.export')->withoutMiddleware(['customer', 'verified', 'unbanned']);
    Route::get('/representatives/payments/generate_qr', [RepresentativePaymentController::class, 'generate_qr'])->name('payments.generate_qr')->withoutMiddleware(['customer', 'verified', 'unbanned']);

    Route::post('/representatives/visits/note', [VisitController::class, 'update_note'])->name('visits.note');

    // Route::get('/download-payment/{id}', 'App\Http\Controllers\PaymentController@downloadPayment')->name('download.payment');

    Route::get('/representatives/payments/share-by-whatsapp/{id}', [RepresentativePaymentController::class, 'shareByWhatsapp'])->name('payments.share-by-whatsapp');
    // Route::post('/product_review_modal', [CustomerController::class, 'product_review_modal'])->name('product_review_modal');
});
...
Route::controller(PersonalInfoController::class)->group(function () {
    Route::get('/personalInfo/create', 'create')->name('create.personalInfo')->middleware('auth');
    Route::post('/personalInfo/store', 'store')->name('store.personalInfo')->middleware('auth');
    Route::get('/personalInfo/show/{user_hash}', 'show')->name('show.personalInfo');
    Route::get('/personalInfo/update', 'update')->name('update.personalInfo')->middleware('auth');
    Route::get('/personalInfo/{filename}', 'downloadQrCode')->name('downloadQrCode.PersonalInfo')->middleware('auth');

});

Route::controller(ConnectionController::class)->group(function (){
    Route::get('/connection/create/{user_hash}', 'create')->name('create.connection');
    Route::post('/connection/store', 'store')->name('store.connection');
    Route::get('/my_connections/show/{user_id}', 'my_connections')->name('show.connections')->middleware('auth');
